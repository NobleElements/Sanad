    ...
    <div className="flex-1 p-8 overflow-y-auto bg-slate-50 dark:bg-slate-900 relative">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex gap-6 mb-8 border-b border-slate-200 dark:border-slate-700">
          <button onClick={() => handleTabChange('shelf')} className={`pb-3 font-medium flex items-center gap-2 ${activeTab==='shelf' ? 'text-indigo-600 dark:text-indigo-400 border-b-2 border-indigo-600' : 'text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300'}`}><Library className="w-4 h-4"/> My Shelf</button>
          <button onClick={() => handleTabChange('search')} className={`pb-3 font-medium flex items-center gap-2 ${activeTab==='search' ? 'text-indigo-600 dark:text-indigo-400 border-b-2 border-indigo-600' : 'text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300'}`}><SearchIcon className="w-4 h-4"/> Find Books</button>
          <button onClick={() => handleTabChange('history')} className={`pb-3 font-medium flex items-center gap-2 ${activeTab==='history' ? 'text-indigo-600 dark:text-indigo-400 border-b-2 border-indigo-600' : 'text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300'}`}><History className="w-4 h-4"/> History</button>
        </div>
        
        {activeTab === 'shelf' && (
          <div>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-200">My Library</h2>
                <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    <div className="relative">
                        <SearchIcon className="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 dark:text-slate-500" />
                        <input
                            type="text"
                            placeholder="Search library..."
                            value={shelfSearch}
                            onChange={(e) => setShelfSearch(e.target.value)}
                            className="pl-9 pr-3 py-1.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none w-48 dark:bg-slate-700 dark:text-slate-100"
                        />
                    </div>
                    <select
                        value={shelfSort}
                        onChange={(e) => setShelfSort(e.target.value)}
                        className="py-1.5 pl-3 pr-8 text-sm border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none dark:bg-slate-700 dark:text-slate-100"
                    >
                    ...
                    <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-lg flex gap-1 ml-2">
                        <button onClick={() => setViewMode('grid')} className={`p-1.5 rounded-md transition ${viewMode === 'grid' ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`} title="Grid View">
                            <LayoutGrid className="w-4 h-4"/>
                        </button>
                        <button onClick={() => setViewMode('list')} className={`p-1.5 rounded-md transition ${viewMode === 'list' ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`} title="List View">
            ...
            {books.length === 0 ? (
                <div className="bg-white dark:bg-slate-800 p-12 rounded-2xl border border-slate-200 dark:border-slate-700 text-center flex flex-col items-center dark:text-slate-100">
                    <Library className="w-12 h-12 text-slate-300 mb-4"/>
                    <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-2">Your shelf is empty</h3>
                    <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-6">Find books or add them manually to start tracking.</p>
                    <button onClick={() => handleTabChange('search')} className="text-indigo-600 dark:text-indigo-400 font-medium hover:underline">Go to Find Books →</button>
                </div>
            ) : (
                        ...
                        <div key={b.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden flex flex-col hover:shadow-lg transition group">
                            <div className="h-48 bg-slate-100 relative">
                                {b.coverUrl ? (
                                    ...
                                    <div className="w-full h-full flex items-center justify-center text-slate-400 dark:text-slate-500">No Cover</div>
                                )}
                            </div>
                            ...
                            <div className="flex items-center gap-4 flex-1 w-full min-w-0">
                                <div className="w-16 h-24 bg-slate-100 shrink-0 rounded overflow-hidden">
                                    {b.coverUrl ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center text-slate-400 dark:text-slate-500 text-[10px]">No Cover</div>
                                    )}
                                </div>
        ...
        {activeTab === 'search' && (
            <div>
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 mb-8 dark:text-slate-100">
                    <h2 className="text-lg font-semibold mb-4 text-slate-800 dark:text-slate-200">Find a Book</h2>
                    <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-4">
                        <div className="relative flex-1">
                            ...
                            <input 
                                className="block w-full pl-10 pr-3 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100"
                                placeholder="Search OpenLibrary or Google Books..."
                                value={query} 
                                onChange={e => setQuery(e.target.value)} 
                            />
                        </div>
                        <button type="submit" disabled={isSearching} className="bg-indigo-600 dark:bg-indigo-500 hover:bg-indigo-700 dark:hover:bg-indigo-600 dark:bg-indigo-500 text-white px-8 py-2.5 rounded-xl transition font-medium flex items-center gap-2 disabled:opacity-50">
                ...
                {searchResults.length > 0 && (
                    <div>
                        <h2 className="text-xl font-semibold mb-6 text-slate-800 dark:text-slate-200">Search Results</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                            {searchResults.map((b, i) => (
                            <div key={i} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-4 rounded-2xl flex flex-col items-center hover:shadow-md transition relative dark:text-slate-100">
                                {b.source && (
                                    <span className="absolute top-2 right-2 text-[10px] uppercase tracking-wider font-bold bg-slate-100 text-slate-500 dark:text-slate-400 dark:text-slate-500 px-2 py-1 rounded">
                                        {b.source}
                                    </span>
                                )}
                                {b.coverUrl ? (
                                    ...
                                    <div className="w-full h-48 bg-slate-100 mb-4 rounded-lg flex items-center justify-center text-slate-400 dark:text-slate-500 border border-slate-200 dark:border-slate-700 text-sm">
                                        No Cover
                                    </div>
                                )}
        ...
        {activeTab === 'history' && (
            <div>
                <h2 className="text-xl font-semibold mb-6 text-slate-800 dark:text-slate-200">Reading History</h2>
                {historyPeriods.length === 0 ? (
                    <div className="bg-white dark:bg-slate-800 p-12 rounded-2xl border border-slate-200 dark:border-slate-700 text-center flex flex-col items-center dark:text-slate-100">
                        <History className="w-12 h-12 text-slate-300 mb-4"/>
                        <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-2">No reading history yet</h3>
                        <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500">Your logged reading progress and finished books will appear here.</p>
                    </div>
                ) : (
                    <div className="flex flex-col gap-6">
                        {historyPeriods.map(p => {
                            ...
                            return (
                            <div key={p.id} className={`bg-white dark:bg-slate-800 border-2 p-1 rounded-2xl shadow-sm hover:shadow-md transition overflow-hidden ${p.status === 'Completed' ? 'border-emerald-100 dark:border-emerald-900/30' : 'border-indigo-100 dark:border-indigo-900/30'}`}>
                                <div className={`p-5 rounded-xl flex flex-col md:flex-row items-center gap-6 ${p.status === 'Completed' ? 'bg-gradient-to-r from-emerald-50 to-white dark:from-emerald-900/20 dark:to-slate-800' : 'bg-gradient-to-r from-indigo-50 to-white dark:from-indigo-900/20 dark:to-slate-800'}`}>
                                    <div className="w-20 h-32 bg-slate-100 shrink-0 rounded-lg overflow-hidden shadow-md">
                                        {p.book.coverUrl ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center text-slate-400 dark:text-slate-500 text-xs">No Cover</div>
                                        )}
                                    </div>
                                    <div className="flex-1 text-center md:text-left">
                                        {p.status === 'Completed' ? (
                                            <div className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400 mb-2 bg-emerald-100/50 px-3 py-1 rounded-full">
                                                <CheckCircle className="w-3.5 h-3.5"/> Completed
                                            </div>
                                        ) : (
                                            <div className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-indigo-600 dark:text-indigo-400 mb-2 bg-indigo-100 dark:bg-indigo-500/20/50 px-3 py-1 rounded-full">
                                        ...
                                        <div className="flex flex-wrap justify-center md:justify-start gap-4 text-sm text-slate-600 dark:text-slate-400 dark:text-slate-500">
                                            <div className="bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 dark:text-slate-100">
                                                <span className="text-slate-400 dark:text-slate-500 block text-xs uppercase tracking-wider mb-0.5">Started</span>
                                                <span className="font-medium">{new Date(p.startDate).toLocaleDateString()}</span>
                                            </div>
                                            <div className="bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 dark:text-slate-100">
                                                <span className="text-slate-400 dark:text-slate-500 block text-xs uppercase tracking-wider mb-0.5">Finished</span>
                                                <span className="font-medium">{p.endDate ? new Date(p.endDate).toLocaleDateString() : 'N/A'}</span>
                                            </div>
                                            {totalDays > 0 && (
                                                <div className="bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 dark:text-slate-100">
                                                    <span className="text-slate-400 dark:text-slate-500 block text-xs uppercase tracking-wider mb-0.5">Duration</span>
                                                    <span className="font-medium">{totalDays} {totalDays === 1 ? 'day' : 'days'}</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                    <div className="md:border-l md:border-slate-200 dark:border-slate-700 md:pl-6 flex flex-col gap-3 justify-center w-full md:w-auto">
                                        <button 
                                            onClick={() => toggleHistory(p.id)} 
                                            className="w-full md:w-auto px-6 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-indigo-300 hover:text-indigo-600 dark:text-indigo-400 dark:hover:text-indigo-400 dark:text-indigo-400 dark:hover:text-indigo-400 text-slate-700 dark:text-slate-300 rounded-xl font-medium transition text-sm flex justify-center items-center gap-2"
                                        >
                                ...
                                {isExpanded && (
                                    <div className={`p-6 bg-slate-50 dark:bg-slate-900 border-t ${p.status === 'Completed' ? 'border-emerald-100' : 'border-indigo-100'} rounded-b-xl flex flex-col md:flex-row gap-8`}>
                                        ...
                                        <div className="flex-1">
                                            ...
                                            {(!p.plans || p.plans.length === 0) ? (
                                                <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 text-sm italic">No plan was created for this book.</p>
                                            ) : (
                                                <div className="flex flex-col gap-3">
                                                    {p.plans.sort((a,b) => a.orderIndex - b.orderIndex).map(plan => {
                                                        ...
                                                        return (
                                                            <div key={plan.id} className={`flex items-center gap-3 p-3 rounded-lg border ${isCompleted ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-100 text-emerald-800' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300'}`}>
                                                                ...
                                                                <div className="flex-1">
                                                                    ...
                                                                    <p className={`text-xs ${isCompleted ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-500'}`}>Pages {plan.startPage} - {plan.endPage}</p>
                                                                </div>
                                                            </div>
                                                        );
                                                    })}
                                        ...
                                        <div className="flex-1">
                                            ...
                                            {(!p.logs || p.logs.length === 0) ? (
                                                <p className="text-slate-500 dark:text-slate-400 dark:text-slate-500 text-sm italic">No logs recorded for this book.</p>
                                            ) : (
                                                <div className="relative border-l-2 border-indigo-200 ml-3 pl-6 space-y-6">
                                                    {p.logs.sort((a,b) => new Date(b.date) - new Date(a.date)).map(log => (
                                                        <div key={log.id} className="relative">
                                                            <div className="absolute -left-[31px] bg-white dark:bg-slate-800 border-2 border-indigo-400 w-4 h-4 rounded-full mt-0.5"></div>
                                                            <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col sm:flex-row justify-between sm:items-center gap-2 dark:text-slate-100">
                                                                <div>
                                                                    <p className="text-sm font-medium text-slate-800 dark:text-slate-200">
                                                                        Read pages <span className="text-indigo-600 dark:text-indigo-400 font-bold">{log.startPage}</span> to <span className="text-indigo-600 dark:text-indigo-400 font-bold">{log.endPage}</span>
                                                                    </p>
                                                                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{new Date(log.date).toLocaleString()}</p>
                                                                </div>
                                                                <div className="bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 px-3 py-1 rounded-lg text-xs font-bold">
