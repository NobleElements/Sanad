...
export default function PlanModal({ period, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 w-full max-w-xl mx-4 p-6 flex flex-col max-h-[90vh] dark:text-slate-100">
        <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-2">Reading Plan: {period.book.title}</h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-6">Plan your reading sessions by chapters or page ranges. Total pages: {period.book.totalPages}</p>
        
        <div className="flex-1 overflow-y-auto pr-2 flex flex-col gap-3">
          {plans.map((p, idx) => (
            <div key={idx} className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700 dark:text-slate-100">
              <input type="text" value={p.title} onChange={e => updatePlan(idx, 'title', e.target.value)} placeholder="Session/Chapter Name" className="flex-1 p-2 border rounded focus:ring focus:border-indigo-500 text-sm dark:bg-slate-700 dark:text-slate-100" />
